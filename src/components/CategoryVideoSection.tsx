...
export default function CategoryVideoSection({ category }: CategoryVideoSectionProps) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      <AnimatePresence>
        {selectedVideo && (
          <VideoModal video={selectedVideo} onClose={() => setSelectedVideo(null)} language={language} />
        )}
      </AnimatePresence>
    </div>
  );
...
function VideoCard({ video, index, language, onSelect }: VideoCardProps) {
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-xl sm:rounded-2xl overflow-hidden cursor-pointer border-2 border-primary/20 hover:border-primary/50 transition-all duration-300 shadow-md hover:shadow-xl"
      onClick={onSelect}
    >
      {/* Thumbnail */}
...
function VideoModal({ video, onClose, language }: VideoModalProps) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
